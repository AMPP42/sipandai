...
export default function DetailPensiun() {
  ...
  const loadDocumentRequirements = async () => {
    ...
    try {
      ...
      setDocumentRequirements(PENSIUN_DOCUMENT_REQUIREMENTS[kategori] || []);
    } catch (error) {
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      ...
      const { error: updateError } = await supabase
        ...
        .update({
          status: 'draft',
          keterangan: `Kategori: Pensiun${additionalNotes ? ` - ${additionalNotes}` : ''}`,
    ...
    try {
      ...
      if (isEditing) {
        const { error: updateError } = await supabase
          ...
          .update({
            status: 'submitted',
            keterangan: `Perbaikan - Diajukan Ulang - Kategori: Pensiun${additionalNotes ? ` - ${additionalNotes}` : ''}`,
            updated_at: new Date().toISOString(),
            progress: 20,
      ...
      toast({
        title: "Error",
        description: isEditing ? "Gagal mengirim ulang usulan pensiun" : "Gagal submit pengajuan pensiun",
        variant: "destructive"
      });
    } finally {
      setIsSubmitting(false);
  ...
  return (
    <div className="container mx-auto py-6 space-y-6">
      ...
      <div className="flex items-center gap-4">
        <Button variant="outline" size="sm" onClick={() => navigate('/apps/reminder-pensiun?tab=list')}>
          <ArrowLeft className="w-4 h-4 mr-2" />
          Kembali
        </Button>
        <div className="flex-1">
          <h1 className="text-3xl font-bold">
            {isEditing ? 'Edit Pengajuan Pensiun' : 'Detail Pengajuan Pensiun'}
          </h1>
